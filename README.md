...
This section is the most important part of the README. Read it before trusting
the tool with anything.

**What neko-auth protects.** The vault **at rest**: a stolen laptop, a copied
backup, a `.db` file lifted off a disk or out of a Time Machine snapshot, with
neko-auth not running. That is the threat model, and it is the whole threat
model.

An attacker holding your vault file learns **how many accounts are in it**.
Every issuer, account name, shared secret, algorithm, digit count, period and
timestamp is encrypted, and the ciphertext is padded so its length does not
give the names away. Guessing the credentials costs one full Argon2id
evaluation per attempt — there is no cheaper check stored anywhere.

The vault is unlocked by **an email address and a password together**, and
**neither is stored in the file**. Not the address, not a hash of it. An
attacker who copies the vault has to guess both halves, and the file tells them
nothing about either. The flip side is stated plainly below: a mistyped address
is indistinguishable from a wrong password, and forgetting which address you
used loses the vault exactly as surely as forgetting the password.

**What neko-auth does not protect against.**

- **Malware, or any other process running as you.** It can read this process's
  memory directly (`ptrace`, `task_for_pid`, `ReadProcessMemory`). Nothing a
  user-space program does can prevent that.
- **A root or kernel-level attacker.** Same reason, more so.
- **Secrets reaching swap.** We do not `mlock`: `RLIMIT_MEMLOCK` is commonly
  64 KiB on Linux, `VirtualLock` is bounded by the working-set quota, and
  neither survives hibernation. Locking a page you cannot reliably lock is a
  false promise, not a defence. **Use full-disk encryption** — FileVault, LUKS,
  BitLocker — and encrypt your swap.
- **System crash reporters.** We disable core dumps at startup, but macOS
  ReportCrash, Windows Error Reporting and systemd-coredump are outside our
  control.
- **Someone rolling your vault back to a genuinely older, complete copy.** A
  vault-wide signature catches deleted rows, inserted rows, and an old file
  with new rows spliced in — but detecting a wholesale rollback would require
  state kept outside the file, which this tool does not have.
- **Your clipboard.** `get -c` puts a code somewhere every process running as
  you can read it, and which the OS may sync to your other devices. The code is
  wiped after 15 seconds, but that is a mitigation, not protection. *(The
  macOS `org.nspasteboard.ConcealedType` and Windows
  `ExcludeClipboardContentFromMonitorProcessing` hints are **not** currently
  set; doing so needs raw platform clipboard APIs that would race with the
  cross-platform library in use.)*

**Zeroization is best effort.** Key material is held in buffers that erase
themselves, using volatile writes and compiler fences — the strongest guarantee
available on stable Rust. It cannot erase register spills or intermediate
copies the optimizer already made.

**There is no recovery.** Forget either the email or the password and the
accounts in that vault are gone permanently. Make an encrypted backup.
