#!/bin/sh
# neko-auth installer for macOS, Linux and WSL.
#
...
# published checksums, puts it on PATH, and stops. It does not create a
# vault, ask for anything, or contact anything but GitHub.
#
# Overrides:
#   NEKO_AUTH_VERSION      tag to install (default: the latest release)
#   NEKO_AUTH_INSTALL_DIR  where to put the binary (default: ~/.local/bin)
#   NEKO_AUTH_NO_PATH=1    install, but leave shell startup files alone
#
# POSIX sh on purpose: this runs under whatever `sh` is, before neko-auth
# exists to tell anyone what it needs.

set -eu

REPO="zoefix/neko-auth"
BIN="neko-auth"
INSTALL_DIR="${NEKO_AUTH_INSTALL_DIR:-$HOME/.local/bin}"
...
# Which file to add PATH to, for the shell the user actually uses.
shell_rc() {
...
add_to_path() {
    rc="$(shell_rc)"
    marker="# added by the neko-auth installer"
...
version="${NEKO_AUTH_VERSION:-$(latest_tag)}"
...
printf '\n'
printf '\033[32m✓\033[0m %s\n' "$("$INSTALL_DIR/$BIN" --version 2>/dev/null || echo "$BIN $version") installed"
printf '\n'
...
printf '\n'
printf '    neko-auth init      # create your vault\n'
printf '    neko-auth           # open it\n'
printf '\n'
dim "There is no recovery: forget the email or the password and the vault is"
dim "gone. Make an encrypted backup once you have added your accounts."
