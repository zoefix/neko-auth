# neko-auth installer for Windows.
#
...
#
# Overrides:
#   $env:NEKO_AUTH_VERSION      tag to install (default: the latest release)
#   $env:NEKO_AUTH_INSTALL_DIR  where to put the binary
#   $env:NEKO_AUTH_NO_PATH=1    install, but leave PATH alone
...
$InstallDir = if ($env:NEKO_AUTH_INSTALL_DIR) {
    $env:NEKO_AUTH_INSTALL_DIR
} else {
...
$Version = $env:NEKO_AUTH_VERSION
if (-not $Version) {
    try {
        $release = Invoke-RestMethod "https://api.github.com/repos/$Repo/releases/latest" `
            -Headers @{ 'User-Agent' = 'neko-auth-installer' }
...
try {
    ...
    # so an upgrade works even with a session open.
    if (Test-Path $dest) {
        $old = "$dest.old"
    ...
    Write-Host ''
    Write-Host '    neko-auth init      # create your vault'
    Write-Host '    neko-auth           # open it'
    Write-Host ''
    Write-Dim 'There is no recovery: forget the email or the password and the vault is'
    Write-Dim 'gone. Make an encrypted backup once you have added your accounts.'
}
finally {
